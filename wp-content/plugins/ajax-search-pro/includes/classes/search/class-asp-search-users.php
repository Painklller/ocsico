...
if (!class_exists('ASP_Search_USERS')) {
    ...
    class ASP_Search_USERS extends ASP_Search {
                ...
                if ($args['user_email_search']) {
                    if ($kw_logic == 'or' || $kw_logic == 'and') {
                        ...
                        if (count($_s) > 0)
                            ...
                            $_like = $s;
                        $parts[] = "( " . $pre_field . $wpdb->users . ".user_email" . $suf_field . " LIKE $pre_like'%" . $_like . "%'$suf_like )";
                    } else {
                        $_like = array();
                        $op = $kw_logic == 'andex' ? 'AND' : 'OR';
                        foreach ($words as $word) {
                    ...
                    foreach ($args['user_search_bp_fields'] as $field_id) {
                        if ($kw_logic == 'or' || $kw_logic == 'and') {
                            ...
                            if (count($_s) > 0)
                                ...
                                $_like = $s;
                            $bp_cf_parts[] = "( $bp_meta_table.field_id = '" . $field_id . "' AND ( " . $pre_field . $bp_meta_table . ".value" . $suf_field . " LIKE $pre_like'%" . $_like . "%'$suf_like ) )";
                        } else {
                            $_like = array();
                            $op = $kw_logic == 'andex' ? 'AND' : 'OR';
                            foreach ($words as $word) {
        ...
        protected function adv_title($title, $id) {
            ...
            $titlefield = w_isset_def($sd['user_search_advanced_title_field'], '');
        ...
        protected function adv_desc($desc, $id) {
            ...
            $descfield = w_isset_def($sd['user_search_advanced_description_field'], '');
